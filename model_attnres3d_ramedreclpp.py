...
3D CT -> MRI 重建、RA-MedReCL++ 与不确定性估计核心实现。
...
@dataclass
class MedReCLConfig:
    """RA-MedReCL++ parameters for reconstruction-guided CT-to-MRI learning."""
    enabled: bool = True
    proj_dim: int = 64
    ...
    structure_contrast_weight: float = 1.0
    latent_alignment_weight: float = 0.35
    image_multiscale_weight: float = 0.25
    gradient_consistency_weight: float = 0.20
    frequency_consistency_weight: float = 0.15
    uncertainty_blend: float = 0.50
    uncertainty_boost: float = 2.0
    adaptive_weight_max: float = 4.0
    hard_region_quantile: float = 0.80
...
class MedReCLLoss(nn.Module):
    ...
    def __init__(self, config: Optional[MedReCLConfig] = None):
        ...
        self._last_latent_alignment_loss = 0.0
        self._last_image_multiscale_loss = 0.0
        self._last_gradient_consistency_loss = 0.0
        self._last_frequency_consistency_loss = 0.0
        self._last_adaptive_weight_mean = 1.0
        self._last_hard_region_fraction = 0.0
    ...
    def forward_components(
        ...
        pred_mri_feats = model.extract_target_features(torch.clamp(pred, 0.0, 1.0), use_teacher=False)
        ...
        pred_mri_projected = model.project_medrecl_target_features(
            pred_mri_feats,
            use_teacher=False,
        )
        ...
        with torch.no_grad():
            ...
            adaptive_map = self._adaptive_reconstruction_weight(
                target_aux=target_aux,
                grad_map=grad_map,
                error_map=error_map,
            )

        contrast_level_losses: List[torch.Tensor] = []
        ...
        if compute_structure:
            contrast_structure_loss = (
                torch.sum(level_weights * torch.stack(contrast_level_losses))
                / torch.sum(level_weights)
                if contrast_level_losses
                else pred.new_zeros(())
            )
            latent_alignment_loss = self._prediction_latent_alignment_loss(
                pred_features=pred_mri_projected,
                target_features=projected["mri_teacher"],
                level_contexts=level_contexts,
                level_weights=level_weights,
            )
            image_multiscale_loss = self._multiscale_image_consistency_loss(
                pred_image=pred,
                target_image=target,
                adaptive_map=adaptive_map,
            )
            gradient_consistency_loss = self._gradient_consistency_loss(
                pred_image=pred,
                target_image=target,
                adaptive_map=adaptive_map,
            )
            frequency_consistency_loss = self._frequency_consistency_loss(
                pred_image=pred,
                target_image=target,
            )
            structure_loss = (
                self.config.structure_contrast_weight * contrast_structure_loss
                + self.config.latent_alignment_weight * latent_alignment_loss
                + self.config.image_multiscale_weight * image_multiscale_loss
                + self.config.gradient_consistency_weight * gradient_consistency_loss
                + self.config.frequency_consistency_weight * frequency_consistency_loss
            )
        else:
            contrast_structure_loss = pred.new_zeros(())
            latent_alignment_loss = pred.new_zeros(())
            image_multiscale_loss = pred.new_zeros(())
            gradient_consistency_loss = pred.new_zeros(())
            frequency_consistency_loss = pred.new_zeros(())
            structure_loss = pred.new_zeros(())
        ...
        self._last_latent_alignment_loss = float(latent_alignment_loss.detach().item())
        self._last_image_multiscale_loss = float(image_multiscale_loss.detach().item())
        self._last_gradient_consistency_loss = float(gradient_consistency_loss.detach().item())
        self._last_frequency_consistency_loss = float(frequency_consistency_loss.detach().item())
        self._last_adaptive_weight_mean = float(adaptive_map.detach().mean().item())
        hard_threshold = torch.quantile(
            adaptive_map.detach().flatten(1),
            q=max(0.0, min(1.0, self.config.hard_region_quantile)),
            dim=1,
        ).view(-1, 1, 1, 1, 1)
        self._last_hard_region_fraction = float(
            (adaptive_map.detach() >= hard_threshold).float().mean().item()
        )
    ...
    def _adaptive_reconstruction_weight(
        self,
        target_aux: torch.Tensor,
        grad_map: torch.Tensor,
        error_map: torch.Tensor,
    ) -> torch.Tensor:
        """Stop-gradient hard-region weights from anatomy edges and current error."""
        cfg = self.config
        blend = max(0.0, min(1.0, float(cfg.uncertainty_blend)))
        score = blend * error_map + (1.0 - blend) * grad_map
        foreground = (target_aux > cfg.background_mu_threshold).float()
        weight = 1.0 + cfg.uncertainty_boost * score
        weight = torch.where(foreground > 0.0, weight, torch.ones_like(weight))
        weight = weight.clamp(1.0, max(1.0, float(cfg.adaptive_weight_max)))
        normalizer = weight.flatten(1).mean(dim=1).view(-1, 1, 1, 1, 1)
        return (weight / normalizer.clamp_min(cfg.eps)).detach()
    ...
    def _weighted_spatial_mean(
        ...
        if weights.shape[-3:] != values.shape[-3:]:
        ...
        denominator = weights.flatten(1).sum(dim=1).clamp_min(self.config.eps)
        return (numerator / denominator).mean()

    def _prediction_latent_alignment_loss(
        self,
        ...
        """Align generated-MRI latent features to frozen true-MRI latents."""
        losses: List[torch.Tensor] = []
        for pred_feature, target_feature, context in zip(
            pred_features,
            target_features,
            level_contexts,
        ):
            cosine = torch.sum(pred_feature.float() * target_feature.detach().float(), dim=1, keepdim=True)
            loss_map = 1.0 - cosine.clamp(-1.0, 1.0)
            losses.append(self._weighted_spatial_mean(loss_map, context["adaptive"]))
        if not losses:
            return level_weights.new_zeros(())
        weights = level_weights.to(device=losses[0].device, dtype=losses[0].dtype)
        weights = weights[:len(losses)]
        return torch.sum(weights * torch.stack(losses)) / weights.sum().clamp_min(self.config.eps)

    def _multiscale_image_consistency_loss(
        self,
        pred_image: torch.Tensor,
        target_image: torch.Tensor,
        adaptive_map: torch.Tensor,
    ) -> torch.Tensor:
        """Hard-region weighted multi-scale MRI intensity and local-stat loss."""
        losses: List[torch.Tensor] = []
        pred_scale = torch.clamp(pred_image.float(), 0.0, 1.0)
        target_scale = torch.clamp(target_image.float(), 0.0, 1.0)
        weight_scale = adaptive_map.float()
        for scale in range(3):
            l1_map = torch.abs(pred_scale - target_scale.detach())
            pred_mean, pred_std = self._local_mean_std(pred_scale, kernel_size=self.config.stats_kernel)
            target_mean, target_std = self._local_mean_std(target_scale.detach(), kernel_size=self.config.stats_kernel)
            stat_map = torch.abs(pred_mean - target_mean) + 0.5 * torch.abs(pred_std - target_std)
            losses.append(
                self._weighted_spatial_mean(l1_map + 0.25 * stat_map, weight_scale)
            )
            if scale == 2 or min(pred_scale.shape[-3:]) < 4:
                break
            pred_scale = F.avg_pool3d(pred_scale, kernel_size=2, stride=2)
            target_scale = F.avg_pool3d(target_scale, kernel_size=2, stride=2)
            weight_scale = F.avg_pool3d(weight_scale, kernel_size=2, stride=2)
        weights = pred_image.new_tensor([1.0, 0.5, 0.25], dtype=pred_image.dtype)[:len(losses)]
        return torch.sum(weights * torch.stack(losses)) / weights.sum().clamp_min(self.config.eps)

    def _gradient_consistency_loss(
        self,
        pred_image: torch.Tensor,
        target_image: torch.Tensor,
        adaptive_map: torch.Tensor,
    ) -> torch.Tensor:
        pred_grad = self._gradient_magnitude(torch.clamp(pred_image.float(), 0.0, 1.0))
        target_grad = self._gradient_magnitude(torch.clamp(target_image.detach().float(), 0.0, 1.0))
        return self._weighted_spatial_mean(torch.abs(pred_grad - target_grad), adaptive_map)

    def _frequency_consistency_loss(
        self,
        pred_image: torch.Tensor,
        target_image: torch.Tensor,
    ) -> torch.Tensor:
        with torch.autocast(device_type=pred_image.device.type, enabled=False):
            pred_freq = torch.fft.rfftn(
                torch.clamp(pred_image.float(), 0.0, 1.0),
                dim=(-3, -2, -1),
                norm="ortho",
            )
            target_freq = torch.fft.rfftn(
                torch.clamp(target_image.detach().float(), 0.0, 1.0),
                dim=(-3, -2, -1),
                norm="ortho",
            )
            distance_sq = torch.abs(pred_freq - target_freq).square()
            focal_weight = distance_sq.detach().clamp_min(1e-12).sqrt()
            max_weight = focal_weight.amax(dim=(-3, -2, -1), keepdim=True)
            focal_weight = focal_weight / max_weight.clamp_min(1e-12)
            dc_mask = torch.ones_like(focal_weight)
            dc_mask[..., 0, 0, 0] = 0.0
            return torch.mean(dc_mask * focal_weight * distance_sq)
    ...
    def weighted_loss(
        ...
        metrics = {
            ...
            "latent_alignment_loss": self._last_latent_alignment_loss,
            "image_multiscale_loss": self._last_image_multiscale_loss,
            "gradient_consistency_loss": self._last_gradient_consistency_loss,
            "frequency_consistency_loss": self._last_frequency_consistency_loss,
            "adaptive_weight_mean": self._last_adaptive_weight_mean,
            "hard_region_fraction": self._last_hard_region_fraction,
    ...
    def _single_case_level_loss(
        ...
        uncertainty_flat = torch.clamp_min(context["adaptive"].reshape(-1) - 1.0, 0.0)
        ...
        hard_q = torch.clamp_min(context["hard_q"].reshape(()).to(zg_flat.dtype) - 1.0, 0.0)

        anchor_idx = self._sample_anchor_indices(
            score_flat=grad_flat + err_flat + uncertainty_flat,
        ...
        anchor_uncertainty = uncertainty_flat[anchor_idx]
        cand_grad = grad_flat[candidate_idx]
        cand_err = err_flat[candidate_idx]
        cand_uncertainty = uncertainty_flat[candidate_idx]
        ...
        tau = torch.clamp(
            cfg.tau0 / (
                1.0
                + cfg.kappa_g * anchor_grad
                + cfg.kappa_r * anchor_err
                + cfg.kappa_u * anchor_uncertainty
            ),
            min=cfg.tau_min,
            max=cfg.tau_max,
        )
        omega = torch.clamp(
            1.0
            + cfg.beta_g * anchor_grad
            + cfg.beta_r * anchor_err
            + cfg.beta_u * anchor_uncertainty,
            min=1.0,
            max=3.0,
        )
        ...
        hard_region_mask = cand_uncertainty.unsqueeze(0) > hard_q
        error_mask = relaxed_hard_base & (
            (cand_err.unsqueeze(0) > err_q80) | hard_region_mask
        )
        ...
        alpha_matrix = torch.clamp(
            ...
            + cfg.rho_u * (anchor_uncertainty[:, None] + cand_uncertainty[None, :]),
            min=1.0,
            max=4.0,
        )
    ...
    def _build_level_context(
        ...
        adaptive_l = F.interpolate(adaptive_map, size=feature_shape, mode=mode, align_corners=False)
        ...
        hard_q = torch.quantile(
            adaptive_l.flatten(1),
            q=self.config.hard_region_quantile,
            dim=1,
        )
        return {
            "grad": grad_l,
...
def train_one_epoch(
    ...
    running_medrecl_latent_alignment_loss = 0.0
    running_medrecl_image_multiscale_loss = 0.0
    running_medrecl_gradient_consistency_loss = 0.0
    running_medrecl_frequency_consistency_loss = 0.0
    running_medrecl_adaptive_weight_mean = 0.0
    running_medrecl_hard_region_fraction = 0.0
    ...
    for batch in loader:
            ...
            if medrecl_criterion is not None:
                ...
                if structure_weight <= 0.0 and appearance_weight <= 0.0:
                    ...
                    medrecl_metrics = {
                        ...
                        "image_multiscale_loss": 0.0,
                        "gradient_consistency_loss": 0.0,
                        "frequency_consistency_loss": 0.0,
        ...
        running_medrecl_latent_alignment_loss += float(
            medrecl_metrics["latent_alignment_loss"]
        )
        running_medrecl_image_multiscale_loss += float(
            medrecl_metrics["image_multiscale_loss"]
        )
        running_medrecl_gradient_consistency_loss += float(
            medrecl_metrics["gradient_consistency_loss"]
        )
        running_medrecl_frequency_consistency_loss += float(
            medrecl_metrics["frequency_consistency_loss"]
        )
        running_medrecl_adaptive_weight_mean += float(
            medrecl_metrics["adaptive_weight_mean"]
        )
        running_medrecl_hard_region_fraction += float(
            medrecl_metrics["hard_region_fraction"]
        )
    ...
    results = {
        ...
        "medrecl_latent_alignment_loss": running_medrecl_latent_alignment_loss / max(1, num_batches),
        "medrecl_image_multiscale_loss": running_medrecl_image_multiscale_loss / max(1, num_batches),
        "medrecl_gradient_consistency_loss": running_medrecl_gradient_consistency_loss / max(1, num_batches),
        "medrecl_frequency_consistency_loss": running_medrecl_frequency_consistency_loss / max(1, num_batches),
        "medrecl_adaptive_weight_mean": running_medrecl_adaptive_weight_mean / max(1, num_batches),
        "medrecl_hard_region_fraction": running_medrecl_hard_region_fraction / max(1, num_batches),
