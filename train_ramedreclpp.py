...
model_attnres3d_ramedreclpp.py 中。
...
MedReCLConfig = net.MedReCLConfig
MedReCLLoss = net.MedReCLLoss
...
def main():
    ...
    # ========== Med-ReCL 训练约束参数 ==========
    # Med-ReCL 是训练阶段的额外表征约束，不是测试阶段的不确定性方法。
    # 它参与 loss，影响权重学习；Moment/MC 则在训练后使用 best.pth 做不确定性推理。
    parser.add_argument("--medrecl_proj_dim", type=int, default=64,
                        help="Med-ReCL 投影维度，默认64")
    parser.add_argument("--medrecl_appearance_start_ratio", type=float, default=0.50,
                        help="Enhanced Med-ReCL MRI appearance alignment start ratio")
    parser.add_argument("--medrecl_appearance_ramp_end_ratio", type=float, default=0.85,
                        help="Enhanced Med-ReCL appearance cosine ramp end ratio")
    parser.add_argument("--medrecl_lambda_x", type=float, default=0.0,
                        help="CT弱正样本权重；RA-MedReCL++主线默认关闭，仅用于消融")
    parser.add_argument("--medrecl_align_weight", type=float, default=0.2,
                        help="PMR-MedReCL 特征对齐损失权重，默认0.2")
    parser.add_argument("--medrecl_structure_contrast_weight", type=float, default=1.0,
                        help="RA-MedReCL++ 原结构对比项权重，默认1.0")
    parser.add_argument("--medrecl_latent_alignment_weight", type=float, default=0.35,
                        help="RA-MedReCL++ 预测MRI latent对齐权重，默认0.35")
    parser.add_argument("--medrecl_image_multiscale_weight", type=float, default=0.25,
                        help="RA-MedReCL++ 多尺度图像一致性权重，默认0.25")
    parser.add_argument("--medrecl_gradient_consistency_weight", type=float, default=0.20,
                        help="RA-MedReCL++ 梯度/边界一致性权重，默认0.20")
    parser.add_argument("--medrecl_frequency_consistency_weight", type=float, default=0.15,
                        help="RA-MedReCL++ 频域一致性权重，默认0.15")
    parser.add_argument("--medrecl_uncertainty_blend", type=float, default=0.50,
                        help="hard区域权重中error与解剖梯度的混合比例，默认0.50")
    parser.add_argument("--medrecl_uncertainty_boost", type=float, default=2.0,
                        help="hard区域自适应权重增强强度，默认2.0")
    parser.add_argument("--medrecl_adaptive_weight_max", type=float, default=4.0,
                        help="hard区域自适应权重上限，默认4.0")
    parser.add_argument("--medrecl_hard_region_quantile", type=float, default=0.80,
                        help="hard区域分位数阈值，默认0.80")
    parser.add_argument("--medrecl_error_negative_weight", type=float, default=0.5,
                        help="error-guided hard negative权重，默认0.5")
    parser.add_argument("--medrecl_beta_u", type=float, default=1.0,
                        help="hard区域对anchor权重omega的增强系数，默认1.0")
    parser.add_argument("--medrecl_rho_u", type=float, default=0.5,
                        help="hard区域对负样本adaptive alpha的增强系数，默认0.5")
    parser.add_argument("--medrecl_kappa_u", type=float, default=0.5,
                        help="hard区域对InfoNCE温度的调节系数，默认0.5")
    parser.add_argument("--medrecl_lambda_structure_max", type=float, default=0.01,
                        help="Enhanced Med-ReCL structure max weight")
    parser.add_argument("--medrecl_lambda_appearance_max", type=float, default=0.004,
                        help="Enhanced Med-ReCL appearance max weight")
    parser.add_argument("--medrecl_recon_only_ratio", type=float, default=0.20,
                        help="训练前多少比例只用重建损失，默认0.20")
    parser.add_argument("--medrecl_structure_ramp_end_ratio", type=float, default=0.60,
                        help="structure contrast余弦爬升结束比例，默认0.60")
    parser.add_argument("--medrecl_contrast_max_ratio", type=float, default=0.15,
                        help="辅助梯度总范数不超过重建梯度范数的比例，默认0.15")
    parser.add_argument("--medrecl_structure_cap_ratio", type=float, default=0.08,
                        help="结构分支梯度相对重建梯度的上限，默认0.08")
    parser.add_argument("--medrecl_appearance_cap_ratio", type=float, default=0.06,
                        help="MRI appearance分支梯度相对重建梯度的上限，默认0.06")
    parser.add_argument("--medrecl_appearance_context_samples", type=int, default=128,
                        help="sampled foreground vectors per appearance scale")
    parser.add_argument("--medrecl_appearance_context_bandwidth", type=float, default=0.5,
                        help="contextual appearance loss bandwidth")
    parser.add_argument("--medrecl_appearance_context_radii", type=int, nargs=3,
                        default=(4, 3, 2),
                        help="local contextual radius for high/mid/low resolution features")
    parser.add_argument("--medrecl_appearance_amplitude_weight", type=float, default=0.60,
                        help="SmoothL1 share inside MRI appearance alignment")
    parser.add_argument("--medrecl_appearance_contextual_weight", type=float, default=0.30,
                        help="local contextual share inside MRI appearance alignment")
    parser.add_argument("--medrecl_appearance_stat_weight", type=float, default=0.10,
                        help="local MRI mean/std share inside appearance alignment")
    parser.add_argument("--medrecl_appearance_stat_std_weight", type=float, default=0.5,
                        help="relative std weight inside local MRI statistic alignment")
    parser.add_argument("--medrecl_teacher_momentum", type=float, default=0.99,
                        help="EMA momentum of the frozen MRI target teacher")
    parser.add_argument("--medrecl_soft_radius", type=int, nargs=3,
                        default=(0, 1, 1), metavar=("RD", "RH", "RW"),
                        help="highest-resolution soft-positive radius; default 0 1 1")
    parser.add_argument("--medrecl_soft_center_prior", type=float, default=0.50,
                        help="minimum mixture weight retained for the registered center voxel")
    parser.add_argument("--medrecl_soft_spatial_eta", type=float, default=0.10,
                        help="spatial penalty used by local soft positives")
    parser.add_argument("--medrecl_false_negative_threshold", type=float, default=0.95,
                        help="similarity above which a negative is downweighted")
    parser.add_argument("--medrecl_false_negative_weight", type=float, default=0.10,
                        help="denominator weight for likely false negatives")
    parser.add_argument("--medrecl_invariance_weight", type=float, default=0.05,
                        help="contrast-invariant MRI structure loss inside L_structure")
    parser.add_argument("--medrecl_invariance_samples", type=int, default=256,
                        help="sampled MRI positions per scale for contrast invariance")
    parser.add_argument("--medrecl_invariance_temperature", type=float, default=0.10,
                        help="InfoNCE temperature for MRI contrast invariance")
    parser.add_argument("--medrecl_contrast_gamma", type=float, nargs=2,
                        default=(0.85, 1.15), metavar=("MIN", "MAX"),
                        help="gamma range for MRI intensity-only structure augmentation")
    parser.add_argument("--medrecl_contrast_scale", type=float, nargs=2,
                        default=(0.90, 1.10), metavar=("MIN", "MAX"),
                        help="scale range for MRI intensity-only structure augmentation")
    parser.add_argument("--medrecl_contrast_bias", type=float, default=0.05,
                        help="absolute bias range for MRI intensity augmentation")
    parser.add_argument("--medrecl_contrast_noise_std", type=float, default=0.01,
                        help="light noise standard deviation for structure invariance")
    parser.add_argument("--medrecl_gradient_balance_interval", type=int, default=10,
                        help="steps between auxiliary/reconstruction gradient checks")
    parser.add_argument("--medrecl_tau0", type=float, default=0.20,
                        help="Med-ReCL 基础温度，默认0.20")
    parser.add_argument("--medrecl_tau_min", type=float, default=0.10,
                        help="Med-ReCL 最小温度，默认0.10")
    parser.add_argument("--medrecl_tau_max", type=float, default=0.30,
                        help="Med-ReCL 最大温度，默认0.30")
    parser.add_argument("--medrecl_anchor_samples", type=int, default=1024,
                        help="每层 anchor 采样数量，默认1024")
    parser.add_argument("--medrecl_normal_samples", type=int, default=256,
                        help="每层普通负样本数量，默认256")
    parser.add_argument("--medrecl_hard_samples", type=int, default=64,
                        help="每类 hard negative 数量，默认64")
    parser.add_argument("--medrecl_easy_bg_samples", type=int, default=64,
                        help="easy/background 低权重样本数，默认64")
    parser.add_argument("--medrecl_candidate_pool", type=int, default=4096,
                        help="Med-ReCL 候选负样本池大小，默认4096")
    parser.add_argument("--no_medrecl", dest="medrecl", action="store_false",
                        help="关闭 Med-ReCL，仅保留原始重建损失")
    ...
    appearance_mix = (
        args.medrecl_appearance_amplitude_weight
        + args.medrecl_appearance_contextual_weight
        + args.medrecl_appearance_stat_weight
    )
    if abs(appearance_mix - 1.0) > 1e-6:
        raise ValueError(
            "Enhanced Med-ReCL appearance component weights must sum to 1.0; "
            f"got {appearance_mix:.6f}"
        )
    if not 0.0 <= args.medrecl_teacher_momentum < 1.0:
        raise ValueError("--medrecl_teacher_momentum must be in [0, 1)")
    if any(radius < 0 for radius in args.medrecl_soft_radius):
        raise ValueError("--medrecl_soft_radius values must be non-negative")
    if not 0.0 <= args.medrecl_soft_center_prior <= 1.0:
        raise ValueError("--medrecl_soft_center_prior must be in [0, 1]")
    if not 0.0 < args.medrecl_false_negative_weight <= 1.0:
        raise ValueError("--medrecl_false_negative_weight must be in (0, 1]")
    if not -1.0 <= args.medrecl_false_negative_threshold <= 1.0:
        raise ValueError("--medrecl_false_negative_threshold must be in [-1, 1]")
    if not 0.0 <= args.medrecl_uncertainty_blend <= 1.0:
        raise ValueError("--medrecl_uncertainty_blend must be in [0, 1]")
    if args.medrecl_adaptive_weight_max < 1.0:
        raise ValueError("--medrecl_adaptive_weight_max must be >= 1")
    if not 0.0 <= args.medrecl_hard_region_quantile <= 1.0:
        raise ValueError("--medrecl_hard_region_quantile must be in [0, 1]")
    if args.medrecl_invariance_samples < 2:
        raise ValueError("--medrecl_invariance_samples must be at least 2")
    if args.medrecl_invariance_temperature <= 0.0:
        raise ValueError("--medrecl_invariance_temperature must be positive")
    ...
    print(f"Med-ReCL 状态: {args.medrecl}")
    print("方法版本: RA-MedReCL++ (reconstruction-aware latent/image/gradient/frequency hard-region learning)")
    print(
        "RA-MedReCL++ 参数: "
        f"radius={tuple(args.medrecl_soft_radius)}, "
        f"FN threshold/weight={args.medrecl_false_negative_threshold}/"
        f"{args.medrecl_false_negative_weight}, "
        f"EMA={args.medrecl_teacher_momentum}, "
        f"invariance={args.medrecl_invariance_weight}, "
        f"inv samples/tau={args.medrecl_invariance_samples}/"
        f"{args.medrecl_invariance_temperature}, "
        f"latent/image/grad/freq={args.medrecl_latent_alignment_weight}/"
        f"{args.medrecl_image_multiscale_weight}/"
        f"{args.medrecl_gradient_consistency_weight}/"
        f"{args.medrecl_frequency_consistency_weight}"
    )
    ...
    model = AttnResCTtoMRI(
        ...
        use_medrecl=args.medrecl,
        medrecl_proj_dim=args.medrecl_proj_dim,
    ).to(device)
    ...
    # Med-ReCL 是训练阶段的辅助对比学习约束：
    #   它会影响参数更新，帮助特征学习；
    #   它不是不确定性输出，不直接产生 variance map。
    medrecl_criterion = None
    if args.medrecl:
        medrecl_cfg = MedReCLConfig(
            enabled=True,
            proj_dim=args.medrecl_proj_dim,
            appearance_start_ratio=args.medrecl_appearance_start_ratio,
            appearance_ramp_end_ratio=args.medrecl_appearance_ramp_end_ratio,
            lambda_x=args.medrecl_lambda_x,
            tau0=args.medrecl_tau0,
            tau_min=args.medrecl_tau_min,
            tau_max=args.medrecl_tau_max,
            align_weight=args.medrecl_align_weight,
            structure_contrast_weight=args.medrecl_structure_contrast_weight,
            latent_alignment_weight=args.medrecl_latent_alignment_weight,
            image_multiscale_weight=args.medrecl_image_multiscale_weight,
            gradient_consistency_weight=args.medrecl_gradient_consistency_weight,
            frequency_consistency_weight=args.medrecl_frequency_consistency_weight,
            uncertainty_blend=args.medrecl_uncertainty_blend,
            uncertainty_boost=args.medrecl_uncertainty_boost,
            adaptive_weight_max=args.medrecl_adaptive_weight_max,
            hard_region_quantile=args.medrecl_hard_region_quantile,
            lambda_R=args.medrecl_error_negative_weight,
            beta_u=args.medrecl_beta_u,
            rho_u=args.medrecl_rho_u,
            kappa_u=args.medrecl_kappa_u,
            lambda_structure_max=args.medrecl_lambda_structure_max,
            lambda_appearance_max=args.medrecl_lambda_appearance_max,
            recon_only_ratio=args.medrecl_recon_only_ratio,
            structure_ramp_end_ratio=args.medrecl_structure_ramp_end_ratio,
            contrast_max_ratio=args.medrecl_contrast_max_ratio,
            structure_cap_ratio=args.medrecl_structure_cap_ratio,
            appearance_cap_ratio=args.medrecl_appearance_cap_ratio,
            appearance_context_samples=args.medrecl_appearance_context_samples,
            appearance_context_bandwidth=args.medrecl_appearance_context_bandwidth,
            appearance_context_radii=tuple(args.medrecl_appearance_context_radii),
            appearance_amplitude_weight=args.medrecl_appearance_amplitude_weight,
            appearance_contextual_weight=args.medrecl_appearance_contextual_weight,
            appearance_stat_weight=args.medrecl_appearance_stat_weight,
            appearance_stat_std_weight=args.medrecl_appearance_stat_std_weight,
            teacher_momentum=args.medrecl_teacher_momentum,
            soft_positive_radii=(tuple(args.medrecl_soft_radius), (0, 0, 0), (0, 0, 0)),
            soft_positive_center_prior=args.medrecl_soft_center_prior,
            soft_positive_spatial_eta=args.medrecl_soft_spatial_eta,
            false_negative_threshold=args.medrecl_false_negative_threshold,
            false_negative_weight=args.medrecl_false_negative_weight,
            invariance_weight=args.medrecl_invariance_weight,
            invariance_samples=args.medrecl_invariance_samples,
            invariance_temperature=args.medrecl_invariance_temperature,
            contrast_gamma_min=args.medrecl_contrast_gamma[0],
            contrast_gamma_max=args.medrecl_contrast_gamma[1],
            contrast_scale_min=args.medrecl_contrast_scale[0],
            contrast_scale_max=args.medrecl_contrast_scale[1],
            contrast_bias=args.medrecl_contrast_bias,
            contrast_noise_std=args.medrecl_contrast_noise_std,
            gradient_balance_interval=args.medrecl_gradient_balance_interval,
            anchor_samples=args.medrecl_anchor_samples,
            normal_negative_samples=args.medrecl_normal_samples,
            hard_negative_samples=args.medrecl_hard_samples,
            easy_background_samples=args.medrecl_easy_bg_samples,
            candidate_pool_size=args.medrecl_candidate_pool,
        )
        medrecl_criterion = MedReCLLoss(medrecl_cfg).to(device)
    # AdamW 根据 loss.backward() 得到的梯度更新卷积权重、归一化参数、注意力模块、
    # Med-ReCL 投影头等所有 requires_grad=True 的参数。
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        train_metrics = train_one_epoch(
            ...
            medrecl_criterion=medrecl_criterion,
        ...
        if args.medrecl:
            ...
            row["train_medrecl_loss"] = f"{train_metrics['medrecl_loss']:.10e}"
            row["train_medrecl_weighted_loss"] = f"{train_metrics['medrecl_weighted_loss']:.8f}"
            row["train_medrecl_weight"] = f"{train_metrics['medrecl_weight']:.8f}"
            row["train_medrecl_structure_weight"] = f"{train_metrics['medrecl_structure_weight']:.8f}"
            row["train_medrecl_appearance_weight"] = f"{train_metrics['medrecl_appearance_weight']:.8f}"
            row["train_medrecl_structure_gradient_scale"] = (
                f"{train_metrics['medrecl_structure_gradient_scale']:.8f}"
            )
            row["train_medrecl_appearance_gradient_scale"] = (
                f"{train_metrics['medrecl_appearance_gradient_scale']:.8f}"
            )
            row["train_medrecl_structure_gradient_ratio"] = (
                f"{train_metrics['medrecl_structure_gradient_ratio']:.8f}"
            )
            row["train_medrecl_appearance_gradient_ratio"] = (
                f"{train_metrics['medrecl_appearance_gradient_ratio']:.8f}"
            )
            row["train_medrecl_appearance_amplitude_loss"] = (
                f"{train_metrics['medrecl_appearance_amplitude_loss']:.8f}"
            )
            row["train_medrecl_appearance_contextual_loss"] = (
                f"{train_metrics['medrecl_appearance_contextual_loss']:.8f}"
            )
            row["train_medrecl_appearance_stat_loss"] = (
                f"{train_metrics['medrecl_appearance_stat_loss']:.8f}"
            )
            row["train_medrecl_latent_alignment_loss"] = (
                f"{train_metrics['medrecl_latent_alignment_loss']:.8f}"
            )
            row["train_medrecl_image_multiscale_loss"] = (
                f"{train_metrics['medrecl_image_multiscale_loss']:.8f}"
            )
            row["train_medrecl_gradient_consistency_loss"] = (
                f"{train_metrics['medrecl_gradient_consistency_loss']:.8f}"
            )
            row["train_medrecl_frequency_consistency_loss"] = (
                f"{train_metrics['medrecl_frequency_consistency_loss']:.8f}"
            )
            row["train_medrecl_adaptive_weight_mean"] = (
                f"{train_metrics['medrecl_adaptive_weight_mean']:.8f}"
            )
            row["train_medrecl_hard_region_fraction"] = (
                f"{train_metrics['medrecl_hard_region_fraction']:.8f}"
            )
            row["train_medrecl_soft_positive_similarity"] = (
                f"{train_metrics['medrecl_soft_positive_similarity']:.8f}"
            )
            row["train_medrecl_false_negative_ratio"] = (
                f"{train_metrics['medrecl_false_negative_ratio']:.8f}"
            )
            row["train_medrecl_invariance_loss"] = (
                f"{train_metrics['medrecl_invariance_loss']:.8f}"
            )
        ...
        if args.medrecl:
            print(
                ...
                f"Med:{train_metrics['medrecl_loss']:.8e} "
                f"MedW:{train_metrics['medrecl_weighted_loss']:.6f} "
                f"StrW:{train_metrics['medrecl_structure_weight']:.4f} "
                f"AppW:{train_metrics['medrecl_appearance_weight']:.4f} "
                f"AppAmp:{train_metrics['medrecl_appearance_amplitude_loss']:.4f} "
                f"AppCx:{train_metrics['medrecl_appearance_contextual_loss']:.4f} "
                f"AppStat:{train_metrics['medrecl_appearance_stat_loss']:.4f} "
                f"Lat:{train_metrics['medrecl_latent_alignment_loss']:.4f} "
                f"MSImg:{train_metrics['medrecl_image_multiscale_loss']:.4f} "
                f"GradC:{train_metrics['medrecl_gradient_consistency_loss']:.4f} "
                f"FreqC:{train_metrics['medrecl_frequency_consistency_loss']:.4f} "
                f"Hard:{train_metrics['medrecl_hard_region_fraction']:.3f} "
                f"SoftPos:{train_metrics['medrecl_soft_positive_similarity']:.3f} "
                f"FN:{train_metrics['medrecl_false_negative_ratio']:.3f} "
                f"Inv:{train_metrics['medrecl_invariance_loss']:.4f} "
                f"StrG:{train_metrics['medrecl_structure_gradient_ratio']:.3f} "
                f"AppG:{train_metrics['medrecl_appearance_gradient_ratio']:.3f} "
