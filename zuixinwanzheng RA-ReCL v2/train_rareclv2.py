...
RA-ReCL v2 完整训练入口：E0 + residual refinement + curriculum multi-scale Region-PatchNCE。

这份脚本只负责“实验流程编排”，真正的网络结构、数据集、损失函数、
矩传播不确定性计算和 MC Dropout 对照都定义在
model_attnres3d_rareclv2.py 中。
...
import model_attnres3d_rareclv2 as net
...
RAReCLV2Config = net.RAReCLV2Config
MultiScaleRegionPatchNCELoss = net.MultiScaleRegionPatchNCELoss
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="./output_rareclv2",
                        help="RA-ReCL v2独立输出目录，避免覆盖E0/RG/精简RA结果")
    ...
    # ========== RA-ReCL v2 ==========
    parser.add_argument("--rareclv2_lambda", type=float, default=0.05,
                        help="curriculum激活后multi-scale Region-PatchNCE权重，默认0.05")
    parser.add_argument("--rareclv2_temperature", type=float, default=0.07,
                        help="PatchNCE temperature，默认0.07")
    parser.add_argument("--rareclv2_warmup_end_ratio", type=float, default=0.20,
                        help="仅重建阶段结束比例，默认0.20")
    parser.add_argument("--rareclv2_middle_end_ratio", type=float, default=0.60,
                        help="top30%%中期阶段结束比例，默认0.60")
    parser.add_argument("--rareclv2_middle_hard_ratio", type=float, default=0.30,
                        help="中期hard区域比例，默认0.30")
    parser.add_argument("--rareclv2_final_hard_ratio", type=float, default=0.20,
                        help="后期hard区域比例，默认0.20")
    parser.add_argument("--rareclv2_feature_dim", type=int, default=64,
                        help="三尺度PatchNCE投影维度，默认64")
    parser.add_argument("--rareclv2_num_patches", type=int, default=256,
                        help="每病例每尺度anchor patch数量，默认256")
    parser.add_argument("--rareclv2_negative_pool", type=int, default=1024,
                        help="每病例每尺度negative候选数量，默认1024")
    parser.add_argument("--rareclv2_level_weights", type=float, nargs=3,
                        default=[0.20, 0.30, 0.50], metavar=("D1", "D2", "D3"),
                        help="浅/中/深三尺度权重，默认0.20 0.30 0.50")
    parser.add_argument("--refiner_channels", type=int, default=8,
                        help="图像域residual refiner隐藏通道，默认8")
    parser.add_argument("--residual_scale", type=float, default=0.10,
                        help="tanh residual最大幅度，默认0.10")
    parser.add_argument("--no_rareclv2", dest="rareclv2", action="store_false",
                        help="关闭curriculum multi-scale Region-PatchNCE")
    parser.add_argument("--no_refiner", dest="refiner", action="store_false",
                        help="关闭residual refinement head")
    parser.set_defaults(rareclv2=True, refiner=True)
    ...
    if args.rareclv2_lambda < 0.0:
        raise ValueError("--rareclv2_lambda must be non-negative")
    if args.rareclv2_temperature <= 0.0:
        raise ValueError("--rareclv2_temperature must be positive")
    if not 0.0 <= args.rareclv2_warmup_end_ratio < args.rareclv2_middle_end_ratio <= 1.0:
        raise ValueError("curriculum ratios must satisfy 0 <= warmup < middle <= 1")
    for name, value in (
        ("--rareclv2_middle_hard_ratio", args.rareclv2_middle_hard_ratio),
        ("--rareclv2_final_hard_ratio", args.rareclv2_final_hard_ratio),
    ):
        if not 0.0 <= value <= 1.0:
            raise ValueError(f"{name} must be in [0, 1]")
    for name, value in (
        ("--rareclv2_feature_dim", args.rareclv2_feature_dim),
        ("--rareclv2_num_patches", args.rareclv2_num_patches),
        ("--rareclv2_negative_pool", args.rareclv2_negative_pool),
    ...
    if any(weight < 0.0 for weight in args.rareclv2_level_weights):
        raise ValueError("--rareclv2_level_weights must be non-negative")
    if sum(args.rareclv2_level_weights) <= 0.0:
    ...
    print(f"RA-ReCL v2 PatchNCE状态: {args.rareclv2}")
    print(f"Residual refiner状态: {args.refiner}")
    print("方法版本: E0 + residual refinement + curriculum multi-scale Region-PatchNCE")
    print(
        "RA-ReCL v2参数: "
        f"lambda={args.rareclv2_lambda}, "
        f"temperature={args.rareclv2_temperature}, "
        f"curriculum={args.rareclv2_warmup_end_ratio}/"
        f"{args.rareclv2_middle_end_ratio}, "
        f"hard middle/final={args.rareclv2_middle_hard_ratio}/"
        f"{args.rareclv2_final_hard_ratio}, "
        f"level_weights={tuple(args.rareclv2_level_weights)}, "
        f"feature_dim={args.rareclv2_feature_dim}, "
        f"patches/negative_pool={args.rareclv2_num_patches}/"
        f"{args.rareclv2_negative_pool}, "
        f"refiner_channels/scale={args.refiner_channels}/{args.residual_scale}"
    )
    ...
    rareclv2_criterion = None
    if args.rareclv2:
        rareclv2_criterion = MultiScaleRegionPatchNCELoss(
            RAReCLV2Config(
                temperature=args.rareclv2_temperature,
                feature_dim=args.rareclv2_feature_dim,
                num_patches=args.rareclv2_num_patches,
                negative_pool_size=args.rareclv2_negative_pool,
                level_weights=tuple(args.rareclv2_level_weights),
                warmup_end_ratio=args.rareclv2_warmup_end_ratio,
                middle_end_ratio=args.rareclv2_middle_end_ratio,
                middle_hard_ratio=args.rareclv2_middle_hard_ratio,
                final_hard_ratio=args.rareclv2_final_hard_ratio,
            )
        ).to(device)
    # AdamW 根据 loss.backward() 得到的梯度更新卷积权重、归一化参数、注意力模块、
    # E0、residual refiner和PatchNCE投影头中所有requires_grad=True的参数。
    ...
    if args.resume:
        ...
        if "optimizer" in ckpt:
            try:
                ...
                print(
                    "[warning] checkpoint optimizer groups do not match the current "
                    "RA-ReCL v2 configuration; model weights were restored, but AdamW "
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        train_metrics = train_one_epoch(
            ...
            rareclv2_criterion=rareclv2_criterion,
            lambda_patchnce=args.rareclv2_lambda,
        ...
        row = {
            ...
            "train_refinement_abs_delta": (
                f"{train_metrics['refinement_abs_delta']:.8f}"
            ),
            "train_refinement_max_abs_delta": (
                f"{train_metrics['refinement_max_abs_delta']:.8f}"
            ),
        ...
        if args.rareclv2:
            ...
            row["train_rareclv2_patchnce_loss"] = (
                f"{train_metrics['rareclv2_patchnce_loss']:.8f}"
            )
            row["train_rareclv2_weighted_patchnce_loss"] = (
                f"{train_metrics['rareclv2_weighted_patchnce_loss']:.8f}"
            )
            for metric_name in (
                "effective_lambda",
                "curriculum_stage",
                "effective_hard_ratio",
                "patchnce_active",
                "hard_region_fraction",
                "sampled_hard_fraction",
                "positive_similarity",
            ):
                row[f"train_rareclv2_{metric_name}"] = (
                    f"{train_metrics[f'rareclv2_{metric_name}']:.8f}"
                )
        ...
        if args.rareclv2:
            print(
                ...
                f"RRP:{train_metrics['rareclv2_patchnce_loss']:.4f} "
                f"RRPW:{train_metrics['rareclv2_weighted_patchnce_loss']:.4f} "
                f"Stage:{train_metrics['rareclv2_curriculum_stage']:.1f} "
                f"Hard:{train_metrics['rareclv2_effective_hard_ratio']:.2f} "
                f"Sampled:{train_metrics['rareclv2_sampled_hard_fraction']:.3f} "
                f"PosSim:{train_metrics['rareclv2_positive_similarity']:.3f} "
                f"RefDelta:{train_metrics['refinement_abs_delta']:.5f} "
