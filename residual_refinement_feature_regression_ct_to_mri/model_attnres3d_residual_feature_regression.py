#!/usr/bin/env python3
"""Residual-refinement CT->MRI model with medical feature-regression support."""
...
class ResidualRefinementHead3D(nn.Module):
    """Small bounded residual head with zero-initialized output kernel."""
    ...
    def __init__(self, hidden_channels: int = 8, residual_scale: float = 0.10) -> None:
        ...
        # Start from zero residual so optimization begins from pure E0 behavior.
...
class AttnResCTtoMRI(e0.AttnResCTtoMRI):
    """E0 backbone + residual refinement and diagnostic feature-access helpers."""
    ...
    def __init__(
        ...
        if out_channels != 1:
            raise ValueError("Feature-regression residual experiment uses single-channel MRI output.")

        # Med-ReCL is intentionally disabled to keep this experiment aligned with
        # the paired reconstruction objective.
        super().__init__(
    ...
    def forward_with_features(
        ...
        if features is None:
            raise RuntimeError("E0 backbone forward failed to return feature dictionary.")
    ...
    def load_state_dict(self, state_dict, strict: bool = True, assign: bool = False):
        """Load legacy checkpoints while safely keeping newly added refiner weights."""
        state_dict = state_dict.copy()
        current_state = self.state_dict()
        for key, value in current_state.items():
            if key.startswith("refiner.") and key not in state_dict:
                state_dict[key] = value.detach().clone()
        return e0.AttnResCTtoMRI.load_state_dict(
            self,
            state_dict,
            strict=strict,
            assign=assign,
