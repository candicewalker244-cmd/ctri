...
RAReCLConfig = net.RAReCLConfig
RAReCLLoss = net.RAReCLLoss
...
def main():
    ...
    parser.add_argument("--save_dir", type=str, default="./output_rarecl",
                        help="精简RA独立输出目录，默认output_rarecl，避免覆盖E0/RG结果")
    ...
    # ========== 精简 RA-ReCL ==========
    # L_total = L_rec + lambda_latent * L_latent + lambda_region * L_region_patchnce。
    parser.add_argument("--rarecl_lambda_latent", type=float, default=0.05,
                        help="prediction/GT latent alignment固定权重，默认0.05")
    parser.add_argument("--rarecl_lambda_region", type=float, default=0.05,
                        help="Region-aware PatchNCE固定权重，默认0.05")
    parser.add_argument("--rarecl_temperature", type=float, default=0.07,
                        help="PatchNCE temperature，默认0.07")
    parser.add_argument("--rarecl_hard_ratio", type=float, default=0.20,
                        help="按当前绝对重建误差选择的hard区域比例，默认0.20")
    parser.add_argument("--rarecl_feature_dim", type=int, default=64,
                        help="两项RA损失共享的投影特征维度，默认64")
    parser.add_argument("--rarecl_latent_samples", type=int, default=256,
                        help="每病例每尺度latent对齐采样位置数，默认256")
    parser.add_argument("--rarecl_region_patches", type=int, default=256,
                        help="每病例每尺度anchor patch数量，默认256")
    parser.add_argument("--rarecl_negative_pool", type=int, default=1024,
                        help="每病例每尺度negative候选数量，默认1024")
    parser.add_argument("--no_rarecl", dest="rarecl", action="store_false",
                        help="关闭全部精简RA损失，仅运行原始E0重建")
    parser.set_defaults(rarecl=True)
    ...
    for name, value in (
        ("--rarecl_lambda_latent", args.rarecl_lambda_latent),
        ("--rarecl_lambda_region", args.rarecl_lambda_region),
    ):
        if value < 0.0:
            raise ValueError(f"{name} must be non-negative")
    if args.rarecl_temperature <= 0.0:
        raise ValueError("--rarecl_temperature must be positive")
    if not 0.0 <= args.rarecl_hard_ratio <= 1.0:
        raise ValueError("--rarecl_hard_ratio must be in [0, 1]")
    for name, value in (
        ("--rarecl_feature_dim", args.rarecl_feature_dim),
        ("--rarecl_latent_samples", args.rarecl_latent_samples),
        ("--rarecl_region_patches", args.rarecl_region_patches),
        ("--rarecl_negative_pool", args.rarecl_negative_pool),
    ):
        if value <= 0:
    ...
    print(f"精简 RA-ReCL 状态: {args.rarecl}")
    print("方法版本: E0 + prediction/GT latent alignment + Region-aware PatchNCE")
    print(
        "精简 RA-ReCL 参数: "
        f"lambda_latent/region={args.rarecl_lambda_latent}/"
        f"{args.rarecl_lambda_region}, "
        f"temperature={args.rarecl_temperature}, "
        f"hard_ratio={args.rarecl_hard_ratio}, "
        f"feature_dim={args.rarecl_feature_dim}, "
        f"latent/region/negative_pool={args.rarecl_latent_samples}/"
        f"{args.rarecl_region_patches}/{args.rarecl_negative_pool}"
    )
    ...
    rarecl_criterion = None
    if args.rarecl:
        rarecl_criterion = RAReCLLoss(
            RAReCLConfig(
                temperature=args.rarecl_temperature,
                hard_ratio=args.rarecl_hard_ratio,
                feature_dim=args.rarecl_feature_dim,
                latent_samples=args.rarecl_latent_samples,
                region_patches=args.rarecl_region_patches,
                negative_pool_size=args.rarecl_negative_pool,
            )
        ).to(device)
    # AdamW 根据 loss.backward() 得到的梯度更新卷积权重、归一化参数、注意力模块、
    # 精简RA共享投影头等所有 requires_grad=True 的参数。
        ...
        if "optimizer" in ckpt:
            try:
                ...
                print(
                    ...
                    "slim RA configuration; model weights were restored, but AdamW "
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        train_metrics = train_one_epoch(
            ...
            rarecl_criterion=rarecl_criterion,
            lambda_latent=args.rarecl_lambda_latent,
            lambda_region=args.rarecl_lambda_region,
        ...
        if args.rarecl:
            ...
            row["train_rarecl_latent_loss"] = (
                f"{train_metrics['rarecl_latent_loss']:.8f}"
            )
            row["train_rarecl_region_loss"] = (
                f"{train_metrics['rarecl_region_loss']:.8f}"
            )
            row["train_rarecl_weighted_latent_loss"] = (
                f"{train_metrics['rarecl_weighted_latent_loss']:.8f}"
            )
            row["train_rarecl_weighted_region_loss"] = (
                f"{train_metrics['rarecl_weighted_region_loss']:.8f}"
            )
            for metric_name in (
                "hard_region_fraction",
                "sampled_hard_fraction",
                "latent_similarity",
                "region_positive_similarity",
            ):
                row[f"train_rarecl_{metric_name}"] = (
                    f"{train_metrics[f'rarecl_{metric_name}']:.8f}"
                )
        ...
        if args.rarecl:
            print(
                ...
                f"Lat/Reg:{train_metrics['rarecl_latent_loss']:.4f}/"
                f"{train_metrics['rarecl_region_loss']:.4f} "
                f"LatW/RegW:{train_metrics['rarecl_weighted_latent_loss']:.4f}/"
                f"{train_metrics['rarecl_weighted_region_loss']:.4f} "
                f"Hard/Sampled:{train_metrics['rarecl_hard_region_fraction']:.3f}/"
                f"{train_metrics['rarecl_sampled_hard_fraction']:.3f} "
                f"Lat/PosSim:{train_metrics['rarecl_latent_similarity']:.3f}/"
                f"{train_metrics['rarecl_region_positive_similarity']:.3f} "
